<?php
	...
	function forum($option)
	{
		foreach ($option['slev'] as $key => $value) {
			switch ($key) {
				case 'create':
					echo forumCreate($option);
					break;
				case 'read':
					echo forumRead($option);
					break;
				case 'update':
					echo forumUpdate($option);
					break;
				case 'delete':
					echo forumDelete($option);
					break;
				case 'detail':
					echo forumDetail($option);
					break;
				default:
					# code...
					break;
	...
	if(isset($_POST['komentar'])){		      
       $ins = mysql_query("insert into ".$com['tbl']."(
       			".$com['id_com_forum'].",
       			".$com['id_forum'].",
       			".$com['id_user'].",
       			".$com['level'].",
	...
	<form action='' class='form-horizontal' method='post'>
    	...
    	while ($i < mysql_num_fields($query)) {
        	...
        	$out .= "<div class=\"control-group\">
							<label for=\"Komentar\" class=\"control-label\">ISI KOMENTAR</label>
							<div class=\"controls\">
								".form_textarea($com['isi'])."
							</div>
						</div>"; 
            $out .= "	 
	            ...
	            <div class=\"form-actions\">
	            	...
	            	<button type=\"submit\" class=\"btn btn-primary\"  name='komentar'>Komentar</button>	            	
	            </div>    
	        </div>
		...
		mysql_free_result($query);
			...
			$out .= "<div class=\"row-fluid\">
					...
					"; 
			//list komentar
			$comment = mysql_query("select * from ".$com['tbl']." 
					where ".$com['id_forum']."='".$_GET['id']."'
					order by ".$com['tgl'].",".$com['waktu']." desc
					");
			while ($fcomment = mysql_fetch_array($comment)) {
				$cekUser=mysql_fetch_array(mysql_query("select * from ".$fcomment[$com['level']]." where id_".$fcomment[$com['level']]." = '".$fcomment[$com['id_user']]."'"));
				$out .= "
					<div class=\"form-actions\">
						<b>".$cekUser['nama_'.$fcomment[$com['level']]]."</b> ".$fcomment[$com['tgl']]." ".$fcomment[$com['waktu']]."<br>
						<hr>
			        	".$fcomment[$com['isi']]."
